...
ap.add_argument("-m", "--model", type=str, default="grid_search",
                help="Models to test, if specified grid search will make a search space with all models")
ap.add_argument("-s", "--scoring", type=str, default="accuracy",
...
if __name__ == "__main__":
    ...
    print("Doing randomized grid search on {model} with scoring: {scoring}".format(model=model_name, scoring=scoring))
    ...
    scores = cross_val_score(grid.best_estimator_, mnist_transform_train, y_train, cv=5,
    ...
    mlu.save(grid, cv_score, clf_report, name=model_name)
